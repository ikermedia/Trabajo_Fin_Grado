...
class ChatBot extends HTMLElement {
  constructor() {
    ...
    this.entorno = "PROD" // Entorno de desarrollo
  }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if(response.status === 502){
            let error = new Error('Lo siento, ha superado el limite de tiempo de espera para la respuesta. Vuelva a intentarlo en 20 segundos.')
            error.code = response.status;
            throw error;

          } else if(response.status === 503) {
            let error = new Error('Lo siento, el servicio no esta disponible en estos momentos.')
            error.code = response.status;
            throw error;
          }
          else if(response.status === 429) {
            let error = new Error('Lo siento, ha superado el número de solicitudes permitidas. Inténtelo más tarde.')
            error.code = response.status;
            throw error;
          }
          
          else if(response.status === 500) {
            let error = new Error('Lo siento, se ha producido un error de servidor.')
            error.code = response.status;
            throw error;
          }
          else {
        ...
        if (Array.isArray(this.messages)) {
          this.messages.push({
            role: "assistant",
            content: error.code === 502 || error.code === 503 || error.code === 429? error.message : "Perdone, parece que hubo un error procesando su mensaje.",
          });
        }
      }
